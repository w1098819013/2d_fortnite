...
//client side model
class Stage {
	constructor(canvas){ //canvas param rid
		this.canvas = canvas;
		...
		*/
		...
		// Client doesn't need to create any actors other than the player.
	...
	animate(){
		//this.step(); // this is done on the server.
		this.draw();
		// Remove zombies
		//this.actors = this.actors.filter(actor => !actor.isZombie); // this is done on the server.
		for (var actor in this.actors){
			if (this.actors[actor].isZombie){
				delete this.actors[actor];
				if(actor in this.actors){
					console.log("Actor was not properly deleted!");
				}
			}
		}
	}
	...
	step(){
		for (var actorKey in this.actors){
			this.actors[actorKey].step();
		}
	}
	...
	draw(){
		...
		for (var actorKey in this.actors){
			this.actors[actorKey].draw(context);
		}
	}
